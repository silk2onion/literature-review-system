...
class ScholarSerpapiCrawler(BaseCrawler):
    ...
    def search_raw(self, query: str, max_results: int = 10, offset: int = 0) -> List[SourcePaper]:
        """
        目前的占位实现：
        - 如果在配置中未启用 SERPAPI_SCHOLAR_ENABLED，直接返回空列表；
        - 即使启用了，也先返回空列表，后续再逐步接入真实 HTTP 调用。
        """
        # 如果配置里关闭了该数据源，直接返回空
        if not getattr(settings, "SERPAPI_SCHOLAR_ENABLED", False):
            return []

        # TODO: 后续在此处补充真正的 SerpAPI 调用逻辑：
        # 1. 读取 settings.SERPAPI_API_KEY / SERPAPI_SCHOLAR_ENGINE
        # 2. 构造请求参数，调用 SerpAPI HTTP 接口
