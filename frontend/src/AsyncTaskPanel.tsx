...
export const AsyncTaskPanel: React.FC<AsyncTaskPanelProps> = ({
  ...
  const handleStartTask = async () => {
    ...
    try {
      ...
      const es = new EventSource(`${API_BASE_URL}/api/reviews/phd/task/${tid}/stream`);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div>
          <h3 style={{ color: '#e2e8f0', margin: 0, fontSize: '16px', fontWeight: 700 }}>
            ⚡ 一键全自动综述生成
          </h3>
        ...
        {task && (
          <span style={{
            ...
            fontSize: '12px',
            fontWeight: 700,
            background: task.status === 'done' ? 'rgba(16,185,129,0.15)' :
                        task.status === 'failed' ? 'rgba(239,68,68,0.15)' :
            ...
            {task.status === 'done' ? '完成' : task.status === 'failed' ? '失败' : '进行中'}
          </span>
        )}
      </div>
      ...
      {!task && (
        <button
          ...
          style={{
            ...
            borderRadius: '8px',
            border: 'none',
            background: starting ? '#334155' : 'linear-gradient(135deg, #8b5cf6, #ec4899)',
            color: '#fff',
            fontWeight: 700,
            fontSize: '15px',
      ...
      {task && (
        <div style={{ marginTop: '16px' }}>
          ...
          <div style={{ color: '#475569', fontSize: '11px', marginBottom: '12px' }}>
            ...
            {task.finished_at && (
              <span style={{ marginLeft: '12px' }}>完成于 {new Date(task.finished_at).toLocaleTimeString()}</span>
            )}
          </div>
            ...
            {task.steps.map((step, idx) => (
              <div
                ...
                style={{
                  ...
                  gap: '10px',
                  padding: '10px 14px',
                  background: step.status === 'pending' ? 'transparent' :
                              step.status === 'done' ? 'rgba(16,185,129,0.05)' :
                              step.status === 'failed' ? 'rgba(239,68,68,0.08)' :
                              'rgba(245,158,11,0.08)',
                  border: `1px solid ${step.status === 'pending' ? '#1e293b' :
              ...
              >
                ...
                <span style={{
                  ...
                  lineHeight: 1,
                  flexShrink: 0,
                  animation: step.status === 'running' || step.status === 'retrying' ? 'spin 1.5s linear infinite' : 'none',
                }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      {step.elapsed !== null && (
                        <span style={{ fontSize: '11px', color: '#475569' }}>{step.elapsed}s</span>
                      )}
                    </div>
                  </div>
          ...
          {showMarkdown && task.full_markdown && (
            <div style={{
              ...
              <ReactMarkdown remarkPlugins={[remarkGfm]}>{task.full_markdown}</ReactMarkdown>
            </div>
          )}
        </div>
      )}
